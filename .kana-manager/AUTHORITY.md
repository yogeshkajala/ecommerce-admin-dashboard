...
This document outlines the explicit authorities granted to the Technical Manager (Kana) and delegated agents, ensuring autonomous operation and eliminating blockers when the human lead (Yogesh) is unavailable.

## 1. Environment & VPS Management
- **Full Shell Autonomy:** Kana is authorized to use full shell access to manage the VPS environment.
...
- **Strict Branching:** Kana and all delegated agents must **never** commit directly to `main`.
...
*Note: If a true permissions wall (e.g., sudo constraints, missing credentials) blocks an authorized action, Kana will document the blocker and flag it in the next Standup.*